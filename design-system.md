...
- **Class cards** (`.class-card`, e.g. `classes.html`) — photo, then a `.class-card__head` row (name on the left, price at the end/right — `display:flex; justify-content:space-between`), then description directly underneath, then CTA. Every card lines up across the whole grid, not just within a row: CSS Grid's default row-stretch only equalizes height *within one row*. `.class-card h3` is clamped to exactly one line (`white-space:nowrap; overflow:hidden; text-overflow:ellipsis`, plus `min-width:0` since it's a flex item) rather than reserving multiple lines — that keeps the description snug against the name with no dead air below a short one, while a name too long to fit (e.g. "Naptime: A Restorative Practice") truncates with an ellipsis. The full name is still in the DOM (screen readers get all of it) and in a `title` attribute (hover tooltip). `.class-card .text-muted` still reserves 3 lines (`min-height`, in `em`s tied to `--lh-normal` so it stays correct if type scale changes) — since that's a floor (not a cap) and text is top-aligned, any leftover space from a short description collects *after* it, and `.class-card .btn`'s `margin-top: auto` (the card is a flex column) pushes all of it above the CTA. `width: 100%` on the CTA spans it edge-to-edge. Keep class descriptions to 3 lines or under at the narrowest 3-up desktop width — anything longer overflows the reserved space and throws off that card's own row.
- **Class filter** (`.class-filter`, `classes.html` only) — a "Show me classes:" label plus a row of toggle chips (`.chip`) built from `CLASS_FILTERS` in `generator/pages.py`: `all` first, then the rest alphabetized by label. Each `.class-card` carries `data-category` (also set in `pages.py`, must match a `CLASS_FILTERS` key), and clicking a chip in `js/main.js` sets `hidden` on every non-matching card and updates an `aria-live="polite"` count (`.class-filter__count`, e.g. "Showing 2 of 18 classes") so screen reader users get the result too. `CLASSES` is kept sorted A-Z by name in `pages.py`, which is what makes the "All A-Z" option correct without any separate sort logic — filtered subsets stay in that same order. One easy-to-reintroduce bug: `.card` sets its own `display: flex`, which is the same specificity as the browser's default `[hidden] { display: none }` rule — without the `.class-card[hidden] { display: none }` override nearby in `styles.css`, `hidden` cards would still render (author styles beat equal-specificity UA styles). If you add `hidden` support to another card type, it needs the same override.
...
- **Instructor bio modal** (`instructors.html` only) — each `.instructor-card`'s bio is clamped to 2 lines (`.instructor-card__bio`, same `line-clamp` technique as the class-card description) followed by a `.link-more` "Read more" button. Clicking it opens a native `<dialog>` (`.instructor-modal`, `showModal()`) populated from `window.INSTRUCTOR_DATA` — a JSON blob `generator/pages.py` embeds via `json.dumps()`, one entry per instructor (full bio, specialties, and typical weekly class `slots`). Native `<dialog>` gives focus-trapping and ESC-to-close for free; `js/main.js` adds closing on backdrop click (`e.target === dialog`, since `.instructor-modal__inner` fills the box with padding so any in-content click targets that instead) and returns focus to the trigger button on close. The Close button has `autofocus` so opening the modal doesn't leave the *whole dialog* wearing the focus ring (what happens by default when nothing inside is focused). **"Upcoming Classes" isn't baked into the HTML as fixed dates** — that would be wrong the day after deploy. Instead, `nextTwoWeekOccurrences()` in `main.js` computes the real calendar dates for each instructor's weekly `slots` (day + time + class name) relative to whenever the page is actually viewed, so it's always showing a genuinely upcoming two weeks. An instructor with `slots: []` (an ownership or guest role, not a weekly class) gets a different message instead of a fabricated schedule. All of this — bios, specialties, and the sample slots — is placeholder content (flagged in the page's own `.notice`); replace with real copy and a real booking calendar before launch.
...
- **Footer** (`.site-footer`, shared markup in `generator/build_site.py`) — deliberately tight: nav links use the normal 44px WCAG touch-target minimum only up to `720px` (`.site-footer a`), above which they drop to `30px` since a mouse, not a thumb, is doing the clicking by then — that alone used to add well over 100px to the tallest footer column (6 links × 44px). Instagram/Facebook are icon-only (`ICON_INSTAGRAM`/`ICON_FACEBOOK` in `build_site.py`, inline SVG in the same stroke style as the contact-page icons) in a fixed 44px circular touch target (`.footer-social a` — deliberately *not* affected by the 720px tightening above, since these stay icon-sized everywhere). The blurb next to the tagline (`.footer-seo`) is written for local SEO — it's on every page via the shared footer, so it's the one paragraph worth keeping keyword-conscious (city, service names) without reading as stuffed.
