...
- **Class cards** (`.class-card`, e.g. `classes.html`) — photo, then a `.class-card__head` row (name on the left, price at the end/right — `display:flex; justify-content:space-between`), then description directly underneath, then CTA. Every card lines up across the whole grid, not just within a row: CSS Grid's default row-stretch only equalizes height *within one row*, so `.class-card h3` reserves 2 lines (so a short name and one that wraps, like "Naptime: A Restorative Practice", both leave the description starting at the same spot) and `.class-card .text-muted` reserves 3 lines (`min-height`, in `em`s tied to `--lh-tight`/`--lh-normal` so it stays correct if type scale changes). Because both are `min-height` (a floor, not a cap) and text is top-aligned, any leftover space from a short name or short description collects *after* that content, and `.class-card .btn`'s `margin-top: auto` (the card is a flex column) pushes all of it above the CTA — so slack always ends up between the description and the button, never between the header and the description. `width: 100%` on the CTA spans it edge-to-edge. Keep class descriptions to 3 lines or under at the narrowest 3-up desktop width — anything longer overflows the reserved space and throws off that card's own row.
